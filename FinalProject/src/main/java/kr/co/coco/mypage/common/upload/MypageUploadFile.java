package kr.co.coco.mypage.common.upload;
...
public class MypageUploadFile {
    ...
    public static void uploadMethod(MultipartFile upload, MyPageDTO myPageInfo) {
        if (!upload.isEmpty()) {
            ...
            try {
                upload.transferTo(filePath.toFile());
            ...
            myPageInfo.setUploadPath(UPLOAD_PATH); // 업로드 경로
            myPageInfo.setUploadName(fileName); // 수정된 파일명
            myPageInfo.setUploadOriginName(originName); // 원본 파일명
        }
    }
}